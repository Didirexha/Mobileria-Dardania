...
const TvDecoration: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTvDecorationProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        // Fetch products and filter by category on the frontend
        console.log('Fetching TV Decoration products from:', API_URL);
        const res = await fetch(API_URL);
        if (!res.ok) {
          throw new Error(`HTTP error! status: ${res.status}`);
        }
        const data: Product[] = await res.json();
        // Filter products by category 'tv-decoration'
        const tvDecorationProducts = data.filter(product => product.category === 'tv-decoration');
        console.log('Fetched and filtered TV Decoration products:', tvDecorationProducts);
        setProducts(tvDecorationProducts);
      } catch (err) {
        console.error('Error fetching TV Decoration products:', err);
        setError(err instanceof Error ? err.message : 'Failed to fetch TV Decoration products');
      } finally {
        setLoading(false);
      }
    };
    fetchTvDecorationProducts();
  }, []);
  ...
  return (
    <section className="pt-32 pb-20 min-h-screen bg-custom-bg">
      <div className="container mx-auto px-4">
        <h2 className="text-4xl font-light text-center mb-12">TV Decoration Collection</h2>
        {loading ? (
          <div className="text-center text-gray-500 text-lg">Loading TV Decoration products...</div>
        ) : error ? (
          <div className="text-center text-red-500 text-lg">Error: {error}</div>
        ) : products.length === 0 ? (
          <div className="text-center text-gray-500 text-lg">No TV Decoration products found.</div>
        ) : (
